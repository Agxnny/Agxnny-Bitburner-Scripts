...
export async function main(ns) {
  ...
  let raw = readRaw(ns), lastRead = 0, lastTailSave = 0, passedOpen = false, queuedAction = null;
  ...
  while (true) {
    ...
    ns.clearLog();
    ns.printRaw(render(h, decorateAll(raw, now), {
      queue: (action) => { queuedAction = action; }, passedOpen,
...
function render(h,s,ui){
  const res=s.resources.snapshot?.data??{}, srv=s.servers.snapshot?.data??{}, sh=s.scriptHealth.snapshot?.data??{}, upd=s.update.snapshot?.data??{}, av=s.activeValidation.snapshot?.data??{};
  const health=overallHealth(s), rooted=(srv.hosts??[]).filter(x=>x.hasAdminRights);
  const jobs=(res.hosts??[]).flatMap(host=>(host.processes??[]).map(p=>({...p,host:host.hostname}))).sort((a,b)=>(b.ramUsed??0)-(a.ramUsed??0)).slice(0,5);
  const installed=upd.installed??{}, testResults=av.results??[], passed=testResults.filter(r=>r.health==="PASS"), outstanding=testResults.filter(r=>r.health!=="PASS");
  return h("div",{style:S.root},
    h("div",{style:S.header},
      ...
      h("div",{style:S.actions},button(h,"▶  Run Active Tests",()=>ui.queue("tests"),true),button(h,"⌁  Script Health Check",()=>ui.queue("health")),button(h,"↻  Refresh",()=>ui.queue("update-check"))),
    ...
    h("div",{style:S.metrics},
      metric(h,"RAM Pool",formatRam(res.total?.maxRam??0),[`Used: ${formatRam(res.total?.usedRam??0)} (${pct(res.total?.usedRam,res.total?.maxRam)})`,`Free: ${formatRam(res.total?.freeRam??0)} (${pct(res.total?.freeRam,res.total?.maxRam)})`],res.total?.usedRam,res.total?.maxRam),
      metric(h,"Rooted Servers",`${srv.rootedCount??rooted.length} / ${srv.hostCount??0}`,[`Rooted (${pct(srv.rootedCount??rooted.length,srv.hostCount??0)})`],srv.rootedCount??rooted.length,srv.hostCount??0),
    ...
    h("div",{style:S.mainGrid},
      panel(h,"Current Jobs (Top 5 by RAM)",jobs.length?table(h,["PID","SCRIPT","HOST","THREADS","RAM"],jobs.map(j=>[j.pid,j.filename,j.host,j.threads,formatRam(j.ramUsed)]),[.7,3,1.5,1,1]):empty(h,"No running jobs")),
...
function metric(h,title,value,lines=[],barValue=null,barTotal=null){return h("div",{style:S.card},h("div",{style:S.label},title),h("div",{style:S.metric},value),...lines.map((x,i)=>h("div",{key:i,style:S.small},x)),barTotal!=null?progressBar(h,barValue,barTotal):null);}
function progressBar(h,value,total){const p=total>0?Math.max(0,Math.min(100,(value??0)/total*100)):0;return h("div",{style:S.barTrack},h("div",{style:{...S.barFill,width:`${p}%`}}));}
...
function rootedServers(h,rooted,resourceHosts){const usage=new Map(resourceHosts.map(x=>[x.hostname,x]));const rows=[...rooted].map(x=>{const r=usage.get(x.hostname)??x;return{host:x.hostname,max:r.maxRam??0,used:r.usedRam??r.ramUsed??0};}).sort((a,b)=>b.used-a.used).slice(0,5);if(!rows.length)return empty(h,"No rooted servers");return h("div",null,table(h,["SERVER","RAM","USED","FREE","USAGE"],rows.map(x=>[x.host,formatRam(x.max),formatRam(x.used),formatRam(Math.max(0,x.max-x.used)),pct(x.used,x.max)]),[2,1,1,1,1]),h("div",{style:{...S.small,marginTop:"8px"}},`Showing ${rows.length}/${rooted.length} rooted servers`));}
...
function pct(value,total){return total>0?`${((value??0)/total*100).toFixed(1)}%`:"0.0%";}
...
header:{display:"grid",gridTemplateColumns:"minmax(300px,1fr) auto 220px",gap:"14px",alignItems:"center",marginBottom:"14px"},brand:{display:"flex",gap:"12px",alignItems:"center"},pulse:{fontSize:"34px",color:"#42e47a"},title:{fontSize:"23px",fontWeight:800,letterSpacing:".2px"},subtitle:{fontSize:"12px",opacity:.7,marginTop:"3px"},actions:{display:"flex",gap:"8px",flexWrap:"wrap",justifyContent:"center"},health:{border:"1px solid",borderRadius:"7px",padding:"10px 14px",display:"flex",gap:"10px",alignItems:"center",justifyContent:"center"},dot:{fontSize:"19px"},
metrics:{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:"10px",marginBottom:"10px"},card:{background:"#081b28",border:"1px solid #174561",borderRadius:"7px",padding:"12px",boxShadow:"inset 0 0 22px #0005"},label:{fontSize:"13px",fontWeight:700,color:"#8fdcff"},metric:{fontSize:"22px",fontWeight:800,margin:"4px 0"},small:{fontSize:"11px",opacity:.72,lineHeight:1.5},mono:{fontFamily:"monospace",fontSize:"11px",opacity:.8,marginTop:"5px"},barTrack:{height:"7px",background:"#102b3d",border:"1px solid #28546e",borderRadius:"99px",overflow:"hidden",marginTop:"8px"},barFill:{height:"100%",background:"linear-gradient(90deg,#25d66d,#4fe88c)",borderRadius:"99px"},
button:{padding:"8px 12px",border:"1px solid #187fc1",borderRadius:"5px",background:"#0a2940",color:"#dcecff",cursor:"pointer",fontWeight:700},primaryButton:{background:"#0c82d9",borderColor:"#29a6ff"},mainGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"},panel:{background:"#081b28",border:"1px solid #174561",borderRadius:"7px",padding:"12px",minHeight:"250px",boxShadow:"inset 0 0 22px #0005"},panelTitle:{fontSize:"15px",fontWeight:800,paddingBottom:"9px",marginBottom:"8px",borderBottom:"1px solid #174561"},
table:{fontFamily:"monospace",fontSize:"11px"},tableHead:{display:"flex",gap:"10px",padding:"7px 6px",color:"#8fdcff",fontWeight:800,borderBottom:"1px solid #174561"},tableRow:{display:"flex",gap:"10px",padding:"8px 6px",borderBottom:"1px solid #12364b"},testRow:{display:"flex",alignItems:"center",gap:"8px",padding:"8px 7px",borderBottom:"1px solid #12364b",fontSize:"12px"},statusMark:{width:"14px"},collapse:{width:"100%",textAlign:"left",padding:"8px",marginTop:"6px",background:"#0b2536",color:"#dcecff",border:"1px solid #174561",borderRadius:"5px",cursor:"pointer",fontWeight:700},emptySmall:{padding:"14px",textAlign:"center",opacity:.6,fontSize:"11px"},panelActions:{display:"flex",gap:"8px",marginTop:"10px"},empty:{minHeight:"170px",display:"flex",flexDirection:"column",gap:"7px",alignItems:"center",justifyContent:"center",opacity:.72},footer:{display:"flex",justifyContent:"space-between",gap:"12px",marginTop:"10px",padding:"7px 2px",fontSize:"10px",opacity:.65}
};
