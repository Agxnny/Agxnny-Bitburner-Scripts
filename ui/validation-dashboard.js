...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (now - lastTailSave >= 250) {
      rememberTail(ns);
      lastTailSave = now;
    }
    const snapshots = decorateSnapshots(rawSnapshots, now);
    ns.clearLog();
    ns.printRaw(renderDashboard(h, snapshots, activeTab, (tab) => { activeTab = tab; }, actions));
...
function decorate(snapshot, maxAgeMs, now) {
  return { snapshot, freshness: classifyFreshness(snapshot, maxAgeMs, now), ageMs: snapshot?.generatedAt ? Math.max(0, now - snapshot.generatedAt) : Infinity };
}
...
function renderDashboard(h, s, activeTab, setTab, actions) {
  ...
  return h("div", { style: rootStyle },
    ...
    h("div", { style: tabRowStyle }, ...tabs.map(([id, label]) =>
      h("button", { key: id, onClick: () => setTab(id), style: { ...tabStyle, ...(activeTab === id ? activeTabStyle : {}) } }, label))),
...
function OverallView({ h, s }) {
  return h(Panel, { h, title: "Whole-stack validation" },
    h("div", null, ...Object.entries(s).map(([name, item]) =>
      h("div", { key: name, style: rowStyle }, h("span", { style: { flex: 1 } }, name), h("b", null, validationHealth(item)), h("span", { style: monoStyle }, age(item.ageMs))))));
}
...
function HostTable({ h, hosts }) {
  ...
  return h("div", { style: tableViewportStyle }, h(TableHeader, { h, cols: ["Host", "Used", "Free", "Total", "Procs"] }),
    ...rows.map((x) => h("div", { key: x.hostname, style: tableRowStyle }, cell(h, x.hostname, 2), cell(h, formatRam(x.usedRam)), cell(h, formatRam(x.freeRam)), cell(h, formatRam(x.maxRam)), cell(h, x.processCount))));
}
...
function ProcessTable({ h, hosts }) {
  ...
  return h("div", { style: processViewportStyle }, h(TableHeader, { h, cols: ["Host", "Script", "Threads", "RAM"] }),
    ...rows.map((x) => h("div", { key: `${x.host}-${x.pid}`, style: tableRowStyle }, cell(h, x.host), cell(h, x.filename, 3), cell(h, x.threads), cell(h, formatRam(x.ramUsed)))));
}
...
const controlRowStyle = { display: "flex", gap: "8px", alignItems: "center", marginBottom: "10px", flexWrap: "wrap" };
const actionButtonStyle = { padding: "7px 11px", border: "1px solid #6a7079", borderRadius: "6px", background: "#23272e", color: "inherit", cursor: "pointer", fontWeight: 700 };
const controlStatusStyle = { opacity: 0.7, fontSize: "11px", marginLeft: "4px" };
...
const manualGridStyle = { display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "8px", marginBottom: "8px" };
...
const tableViewportStyle = { maxHeight: "330px", overflowY: "auto", paddingRight: "4px" };
const processViewportStyle = { maxHeight: "220px", overflowY: "auto", paddingRight: "4px" };
const tableHeaderStyle = { display: "flex", gap: "8px", padding: "5px 0", fontWeight: 700, opacity: 0.9, position: "sticky", top: 0, zIndex: 1, background: "#191b20", borderBottom: "1px solid #454950" };
...
const revisionStyle = { ...monoStyle, maxWidth: "68%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" };
