...
export async function main(ns) {
  ...
  const status = failures.length === 0 ? HEALTH.PASS : HEALTH.FAIL;
...
function summarize(measurements) {
  ...
  return {
    ...
    persistentRam: valid.filter((item) => item.lifecycle === "persistent").reduce((sum, item) => sum + item.ramPerThread, 0),
    transientRam: valid.filter((item) => item.lifecycle !== "persistent").reduce((sum, item) => sum + item.ramPerThread, 0),
    workerCosts: Object.fromEntries(valid.filter((item) => item.worker).map((item) => [item.path, item.ramPerThread])),
  };
}
...
function publishFailure(ns, stackVersion, startedAt, reason) {
  publishDomainState(ns, PATHS.ramAudit, {
    ...
    source: ns.getScriptName(),
    valid: false,
    data: { status: HEALTH.FAIL, stackVersion, generatedAt: Date.now(), durationMs: Date.now() - startedAt, reason },
  });
}
...
function printReport(ns, data) {
  ...
  if (data.requiredFailures.length > 0) ns.tprint(`Required measurement failures: ${data.requiredFailures.join(", ")}`);
  const largest = [...data.measurements].filter((item) => item.valid).sort((a, b) => b.ramPerThread - a.ramPerThread).slice(0, 10);
  for (const item of largest) ns.tprint(`${formatRam(item.ramPerThread).padStart(10)}  ${item.path}`);
}

function formatRam(value) { return `${Number(value || 0).toFixed(2)} GB`; }
