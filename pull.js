...
const TERMINAL_ALIASES = [
  ["teststart", "run /validation/start.js"],
  ["tests", "run /validation/active-tests.js"],
  ["health", "run /validation/script-health.js"],
  ["ramaudit", "run /systems/ram-audit/audit.js"],
  ["update", "run /pull.js"],
  ["repair", "run /pull.js --repair"],
];
...
export async function main(ns) {
  ...
  if (pullerStaged) {
    ...
    const handoffPid = ns.exec(HANDOFF_PATH, "home", 1, "--old-pid", ns.pid, "--staged", STAGED_PULL, "--target", PULL_PATH,
      "--installed-state", INSTALLED_STATE, "--record", JSON.stringify(record), "--runtime", "[]");
...
function installAliases(ns, target) {
  const aliases = [...TERMINAL_ALIASES];
  if ((target.files ?? []).some((file) => canonicalPath(file.path) === "/start.js")) aliases.unshift(["start", "run /start.js"]);
  for (const [name, substitution] of aliases) {
    try { ns.ui.alias(name, substitution, true); }
    catch (error) { ns.tprint(`WARNING ALIAS_FAILED ${name}: ${String(error)}`); }
  }
  ns.tprint(`Aliases ready: ${aliases.map(([name]) => name).join(", ")}`);
}
...
async function stopHomeNonPersistent(ns, persistent) {
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    ...
    for (const p of candidates) { ns.tprint(`  STOP HOME ${p.filename} pid=${p.pid}`); ns.kill(p.pid); }
    await ns.sleep(100);
  }
...
async function stopRemoteWorkers(ns, workers) {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    ...
    if (candidates.length === 0) return { ok: true, survivors: [] };
    for (const p of candidates) { ns.tprint(`  STOP WORKER ${p.host} ${p.filename} pid=${p.pid}`); ns.kill(p.pid, p.host); }
    await ns.sleep(100);
  }
  const survivors = collectRemoteWorkers(ns, hosts, workers);
  return { ok: survivors.length === 0, survivors };
}
function collectRemoteWorkers(ns, hosts, workers) { const result = []; for (const host of hosts) for (const p of ns.ps(host)) if (workers.has(canonicalPath(p.filename))) result.push({ host, ...p }); return result; }
function discoverNetwork(ns) { const seen = new Set(["home"]), queue = ["home"]; for (let i = 0; i < queue.length; i += 1) for (const next of ns.scan(queue[i])) if (!seen.has(next)) { seen.add(next); queue.push(next); } return queue; }
function closeAllHomeTails(ns) { for (const p of ns.ps("home")) { if (p.pid === ns.pid) continue; const running = ns.getRunningScript(p.pid, "home"); if (!running?.tailProperties) continue; try { ns.ui.closeTail(p.pid); } catch {} } }
function canonicalPath(path) { const value = String(path ?? "").trim(); return value.startsWith("/") ? value : `/${value}`; }
function buildPlan(ns, installed, target, repair) { const installedByPath = new Map((installed?.files ?? []).map((f) => [f.path, f])); const fetch = [], unchanged = []; for (const targetFile of target.files) { const current = installedByPath.get(targetFile.path); const missing = !ns.fileExists(targetFile.path, "home"); const changed = !current || current.fileVersion !== targetFile.fileVersion; if (!installed || missing || changed || repair) fetch.push(targetFile); else unchanged.push(targetFile.path); } return { fetch, unchanged, remove: [...(target.removedFiles ?? [])] }; }
...
function validateManifest(m) { const errors = []; if (!m || typeof m !== "object") return { valid: false, errors: ["manifest must be an object"] }; if (!Number.isInteger(m.schemaVersion)) errors.push("schemaVersion missing/invalid"); if (!Number.isInteger(m.revisionSequence)) errors.push("revisionSequence missing/invalid"); if (typeof m.revisionId !== "string" || !m.revisionId) errors.push("revisionId missing/invalid"); if (typeof m.releaseVersion !== "string" || !m.releaseVersion) errors.push("releaseVersion missing/invalid"); if (!Array.isArray(m.files)) errors.push("files missing/invalid"); if (!Array.isArray(m.runtimeEntries)) errors.push("runtimeEntries missing/invalid"); if (!Array.isArray(m.removedFiles)) errors.push("removedFiles missing/invalid"); if (!Array.isArray(m.persistent)) errors.push("persistent missing/invalid"); if (Array.isArray(m.files)) { const seen = new Set(); for (const file of m.files) { if (!file || typeof file.path !== "string" || !file.path.startsWith("/")) errors.push("invalid file path"); else if (seen.has(file.path)) errors.push(`duplicate file path ${file.path}`); else seen.add(file.path); if (!Number.isInteger(file.fileVersion) || file.fileVersion < 1) errors.push(`invalid fileVersion ${file.path ?? "?"}`); } } return { valid: errors.length === 0, errors }; }
