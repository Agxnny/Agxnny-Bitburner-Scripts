...
export function appendBatchHistory(history, complete) {
    ...
    if (current.samples.some((entry) => String(entry?.batchId ?? "") === sample.batchId)) return state;
...
export function normalizeHistory(history) {
    ...
    return {
        version: 2,
        model: "ROLLING_BATCH_HISTORY_V2_PIPELINE_EVIDENCE",
...
export function getTargetBatchSafety(history, hostname) {
    ...
    if (!target) {
        return {
            ...
            recommendedDepth: 1,
            confidence: "UNPROVEN",
            reason: "No real pipeline completion evidence yet",
        };
    }
    return target;
}
...
function makeSample(complete) {
    ...
    return {
        ...
        pipeline: Boolean(complete?.pipeline),
        sourceModel: String(complete?.model ?? ""),
        healthy,
        orderCorrect,
        missingJobs,
        moneyPercent,
...
function summarizeTarget(hostname, samples) {
    const pipelineSamples = samples.filter((sample) => sample?.pipeline === true);
    let consecutiveClean = 0;
    for (let i = pipelineSamples.length - 1; i >= 0; i -= 1) {
        if (!pipelineSamples[i]?.healthy) break;
        consecutiveClean += 1;
    }

    const cleanSamples = samples.filter((sample) => sample?.healthy).length;
    const cleanPipelineSamples = pipelineSamples.filter((sample) => sample?.healthy).length;
    ...
    return {
        ...
        pipelineSampleCount: pipelineSamples.length,
        cleanSamples,
        ...
        maxSecurityDelta,
        reason: pipelineSamples.length === 0
            ? "No real pipeline completion evidence yet"
            : pipelineSamples[pipelineSamples.length - 1]?.healthy
                ? `${consecutiveClean} consecutive clean real pipeline batch(es)`
                : "Latest real pipeline batch failed safety history criteria",
    };
}
