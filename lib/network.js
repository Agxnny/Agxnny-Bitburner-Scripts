...
/**
 * Analyze root access and HGW eligibility separately.
 * Rooting is gated by port access; HGW also requires sufficient hacking level.
 * This function is read-only: it never opens ports or calls NUKE.
 *
 * @param {NS} ns
...
export function analyzeServerAccess(ns, hostname) {
    ...
    const canRootNow = hasRoot || !portsBlocked;
    const canHackNow = hasRoot && !levelBlocked;
    const canBecomeHackableNow = canRootNow && !levelBlocked;
 ...
 * Hacking level is deliberately not part of the rooting requirement.
...
export function tryRootServer(ns, hostname) {
    ...
    if (access.ports.available >= access.ports.required) {
        ns.nuke(hostname);
    }
