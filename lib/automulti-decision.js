...
function scoreCandidate(candidate, objective) {
    ...
    const safetyFactor = clamp(Number(candidate.safetyFactor ?? 1), 0.25, 1.10);
    const rawScore = weights.money * (moneyRate + moneyEfficiency * 0.05)
    ...
    return {
        ...
        xpEfficiency,
        safetyFactor,
        objectiveScore: rawScore * safetyFactor,
    };
}
