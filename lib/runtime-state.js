...
/** @param {NS} ns @param {object} state */
export function publishPrepperState(ns, state) {
    replaceSnapshot(ns, RuntimePort.PREPPER_STATE, state);
}

/** @param {NS} ns */
export function readPrepperState(ns) {
    return readSnapshot(ns, RuntimePort.PREPPER_STATE);
}
