 ...
 * Pass a precomputed network analysis to avoid rescanning when a caller already
 * has one, such as the short-lived planner.
 *
 * @param {NS} ns
 * @param {object[]|null} servers
 */
export function getEligibleTargetHostnames(ns, servers = null) {
    const network = servers ?? analyzeNetwork(ns);
    return network
 ...
 * @param {object[]|null} servers
 */
export function rankEligibleTargets(ns, servers = null) {
    return getEligibleTargetHostnames(ns, servers)
