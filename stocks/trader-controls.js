...
export function TraderControls({ config, traderState, traderRunning, onSave }) {
    ...
    return el("div", { style: { ...s.card, marginBottom: "7px" } },
        ...
        el("div", { style:s.stats }, stat("Configured",configured), stat("Live limit",money(state.capitalLimit??0)), stat("Exposure",money(state.exposure??0)), stat("Short exposure",money(state.shortExposure??0)), stat("Short limit",money(state.shortCapitalLimit??0)), stat("Stop loss",`${Number(source.stopLossPercent??0).toFixed(1)}%`)),
