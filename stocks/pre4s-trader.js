...
export async function main(ns) {
    ...
    if (!quiet) ns.tprint(`[PRE4S] LIVE · dashboard capital control enabled · per-symbol ${(MAX_SYMBOL_FRACTION * 100).toFixed(0)}% · shorts ${allowShort ? "ON" : "OFF"}`);
    ...
    while (true) {
        ...
        for (const signal of ranked) {
            ...
            if (p.longShares > 0 && signal.score < EXIT_SCORE) {
                const sale = safe(() => ns.stock.sellStock(p.symbol, p.longShares), 0);
                if (sale > 0) { realized += (sale - p.longAverage) * p.longShares - COMMISSION; trades += 1; }
            }
            if (p.shortShares > 0 && signal.score > -EXIT_SCORE) {
                const cover = safe(() => ns.stock.sellShort(p.symbol, p.shortShares), 0);
                if (cover > 0) { realized += (p.shortAverage - cover) * p.shortShares - COMMISSION; trades += 1; }
            }
        }
        ...
        await ns.write(STATE_FILE, JSON.stringify({
            model: "PRE4S_TRADER_V2_CAPITAL_CONTROL",
            status: "LIVE",
            allowShort,
